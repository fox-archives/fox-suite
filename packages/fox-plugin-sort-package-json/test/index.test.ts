...
const testPackageJsons = fs
	.readdirSync(path.join(__dirname, './fixtures'))
	.filter(
		(fileName) =>
			fileName.includes('package.json') && !fileName.includes('fmt')
	)
...
for (const testPackageJson of testPackageJsons) {
	test(`ensure ${testPackageJson}.package.json is ordered properly`, async (t) => {
		const packageJsonLocation = path.join(
			__dirname,
			`./fixtures/${testPackageJson}.package.json`
		);
		const correctPackageJsonLocation = path.join(
			__dirname,
			`./fixtures/${testPackageJson}.fmt.package.json`
		);

		const packageJsonRaw = await fs.promises.readFile(packageJsonLocation, {
			encoding: 'utf8',
		});
		const correctPackageJsonRaw = await fs.promises.readFile(
			correctPackageJsonLocation,
			{ encoding: 'utf8' }
		);
