...
export function getPluginInfo(): { pluginPath: string; pluginName: string } {
	...
	if (!pluginPath || !pluginName) {
		throw new Error(
			'either pluginPath or pluginName are falsey. this is a problem',
		)
	}
...
export function getBinDirents(pluginPath: string): Promise<Dirent[]> {
	return fs.promises.readdir(path.join(pluginPath, 'bin'), {
		withFileTypes: true,
	})
}
...
export async function doesWrongTemplateFolderExist(
	pluginPath: string,
): Promise<boolean> {
...
export async function babelTraverse(
	traverseOption: any,
): Promise<babel.types.File | babel.types.Program | null> {
	const { pluginPath } = getPluginInfo()
	const indexFile = path.join(pluginPath, 'src/index.ts')
	const fileContents = await fs.promises.readFile(indexFile, {
		encoding: 'utf8',
	})
