...
export async function fixFunction(): Promise<void> {
	await foxUtils.buildFix({
		...
		async fn() {
			...
			const config = (await import(path.join(__dirname, '../config/prettier.config.js'))).default
			...
			for(const file of files) {
				fs.promises.readFile(file, { encoding: 'utf8' })
					.then(async (content: string): Promise<any> => {
						try {
							const formatedContent = prettier.format(content, {
								...config,
								filepath: file
							})
							...
							return fs.promises.writeFile(file, formatedContent, {
								mode: stats.mode
							})
						} catch (err) {
							throw new Error(err)
					...
					.catch((err: any) => {
						if (!err.toString().includes('Error: No parser could be inferred for file')) {
							console.error(err)
						}
					})
			}
